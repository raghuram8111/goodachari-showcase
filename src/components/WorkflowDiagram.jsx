...
export default function WorkflowDiagram() {
  ...
  return (
    <section id="workflow" className="py-24 bg-[#080c14] border-b border-slate-800/60">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {/* 9-Step Horizontal Navigation Strip - Clean & Subtle */}
        <div className="mb-10 overflow-x-auto pb-2">
          <div className="flex items-center gap-2 min-w-[780px] border-b border-slate-800/60 pb-3">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 mb-14">
          {/* Left Detail Panel */}
          <div className="lg:col-span-7 p-7 rounded-xl bg-slate-900/70 border border-slate-800 space-y-6">
            ...
            {/* Technical Mechanism */}
            <div className="p-4 rounded-lg bg-slate-950/80 border border-slate-800/80 space-y-1 text-xs">
              <span className="font-mono font-semibold text-cyan-400 uppercase tracking-wider block">
                Technical Mechanism:
              </span>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 pt-2 font-mono text-xs">
              <div className="space-y-1">
                ...
                <div className="text-slate-200 bg-slate-950/60 p-2.5 rounded border border-slate-800/60">
                  {activeStep.inputs}
                </div>
              </div>
              <div className="space-y-1">
                <span className="text-slate-400 uppercase text-[10px]">Produced Output:</span>
                <div className="text-cyan-300 bg-slate-950/60 p-2.5 rounded border border-slate-800/60">
                  {activeStep.output}
                </div>
              </div>
            </div>
            ...
            <div className="flex items-center justify-between pt-4 border-t border-slate-800 font-mono text-xs">
              <button
                ...
                className="px-3 py-1.5 rounded bg-slate-800 hover:bg-slate-700 disabled:opacity-30 text-slate-300 transition-colors"
              >
                ← Previous Step
              </button>
              ...
              <button
                ...
                className="px-3 py-1.5 rounded bg-cyan-500 hover:bg-cyan-400 disabled:opacity-30 text-slate-950 font-bold transition-colors"
              >
                Next Step →
              </button>
            </div>
          ...
          <div className="lg:col-span-5 p-5 rounded-xl bg-slate-900/40 border border-slate-800">
              ...
              {PROTOTYPE_WORKFLOW_STEPS.map((s, idx) => (
                ...
                >
                  <div className="flex items-center gap-3">
                    <span className="text-slate-500 font-semibold">{s.number}</span>
                    <span>{s.title}</span>
                  </div>
                  {idx === 8 && (
                    <span className="text-[10px] text-emerald-400">Verdict</span>
                  )}
                </div>
              ))}
            </div>
        ...
        {/* DECISION STATES BLOCK - Clean, Spacious */}
        <div className="rounded-xl bg-slate-900/40 border border-slate-800 p-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {/* Confirmed */}
            <div className="p-5 rounded-lg bg-slate-950/60 border border-slate-800 space-y-2.5">
